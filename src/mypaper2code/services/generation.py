...
from mypaper2code.services.analysis import load_research_understanding
from mypaper2code.services.decisions import load_decisions, unresolved_blocking_ambiguities
...
class CodeWriter:
    def implement(self, workspace: Path) -> Path:
        ...
        if research_plan_path.exists():
            plan = load_model(research_plan_path, ResearchImplementationPlan)
            return self._implement_research_plan(workspace, plan)
        ...
        if plan_path.exists():
            ...
            if isinstance(plan, ResearchImplementationPlan):
                return self._implement_research_plan(workspace, plan)
    ...
    def _implement_research_plan(self, workspace: Path, plan: ResearchImplementationPlan) -> Path:
        understanding = load_research_understanding(workspace)
        unresolved = unresolved_blocking_ambiguities(understanding, workspace)
        if unresolved:
            ids = ", ".join(item.ambiguity_id for item in unresolved)
            raise RuntimeError(f"Blocking decisions must be resolved before implementation: {ids}")

        root = workspace / "generated"
        compat_root = workspace / "generated_code"
        root.mkdir(parents=True, exist_ok=True)
        compat_root.mkdir(parents=True, exist_ok=True)
        decisions = load_decisions(workspace)
        files = _agentic_project_files(plan, {key: value.value for key, value in decisions.items()})
        written = self._write_many(root, files)
        self._write_many(compat_root, files)
        write_json(
            root / "evidence" / "manifest.json",
            [item.model_dump(mode="json") for item in plan.understanding.evidence],
        )
        write_json(
            compat_root / "evidence" / "manifest.json",
            [item.model_dump(mode="json") for item in plan.understanding.evidence],
        )
        trace = AgenticImplementationTrace(
            entries=[
                AgenticTraceEntry(
                    ...
                    implemented_in=[
                        CodeLocation(file=relative, symbol=symbol)
                        for relative, symbol in written
                        if relative in step.target_files or relative.startswith("tests/")
                    ],
                    assumptions=step.risks,
                    reviewer_notes=[
                        "Generated by the local agentic scaffold writer; "
                        "review required for full fidelity."
                    ],
                )
                for step in plan.steps
            ]
        )
        write_json(workspace / "trace" / "implementation_trace.json", trace.model_dump(mode="json"))
        write_json(
            workspace / "analysis" / "implementation_trace.json",
            _legacy_trace(trace).model_dump(mode="json"),
        )
        return root
...
def _agentic_project_files(
    plan: ResearchImplementationPlan,
    decisions: dict[str, str],
) -> dict[str, str]:
    requirements = ["pyyaml\n"]
    if plan.paper_type == "ml":
        requirements.append("# Optional for ML adapters generated from ML papers:\ntorch\n")
    return {
        "requirements.txt": "".join(requirements),
        "README.md": _agentic_readme(plan, decisions),
        "configs/default.yaml": _agentic_config(plan, decisions),
        "src/method/__init__.py": (
            "from src.method.core import MethodCore\n\n__all__ = ['MethodCore']\n"
        ),
        "src/method/core.py": _method_core_code(plan),
        "src/method/ml_adapter.py": (
            _ml_adapter_code() if plan.paper_type == "ml" else _empty_module()
        ),
        "src/experiments/__init__.py": "",
        "src/experiments/protocol.py": _protocol_code(),
        "scripts/run_experiment.py": _run_experiment_script(),
        "scripts/train.py": _compat_train_script(),
        "scripts/evaluate.py": _compat_evaluate_script(),
        "tests/test_smoke.py": _agentic_smoke_test(),
        "tests/test_contract.py": _agentic_contract_test(),
    }


def _agentic_config(plan: ResearchImplementationPlan, decisions: dict[str, str]) -> str:
    import yaml

    data = {
        "schema_version": plan.schema_version,
        "paper_type": plan.paper_type,
        "datasets": plan.understanding.datasets,
        "metrics": plan.understanding.metrics,
        "protocols": plan.understanding.protocols,
        "hyperparameters": plan.understanding.hyperparameters,
        "expected_artifacts": plan.understanding.expected_artifacts,
        "decisions": decisions,
    }
    return yaml.safe_dump(data, sort_keys=False)


def _agentic_readme(plan: ResearchImplementationPlan, decisions: dict[str, str]) -> str:
    steps = "\n".join(
        f"- `{step.step_id}`: {step.title} -> {', '.join(step.target_files)}"
        for step in plan.steps
    )
    evidence = "\n".join(
        f"- `{item.evidence_id}` ({item.kind}, page {item.page}): {item.text[:180]}"
        for item in plan.understanding.evidence[:8]
    )
    decision_lines = "\n".join(f"- `{key}`: {value}" for key, value in decisions.items())
    return f"""# Generated Research Implementation

This project is an agentic first implementation scaffold. It is designed to be
reviewed and extended before claiming scientific reproduction.
...
{_markdown_list(plan.understanding.contributions)}
...
{decision_lines or "- No human decisions were recorded."}
...
def _method_core_code(plan: ResearchImplementationPlan) -> str:
    algorithms = repr(plan.understanding.algorithms)
    equations = repr(plan.understanding.equations)
    metrics = repr(plan.understanding.metrics)
    return f'''from __future__ import annotations
...
@dataclass
class MethodCore:
    """Paper-derived method facade with evidence-backed metadata."""
    ...
    def __post_init__(self) -> None:
        self.algorithms = self.algorithms or {algorithms}
        self.equations = self.equations or {equations}
...
from src.method import MethodCore
...
def run_protocol(config: dict[str, Any]) -> dict[str, Any]:
    core = MethodCore(metrics=config.get("metrics", []))
    return core.run(
        {
...
def _run_experiment_script() -> str:
    return '''from __future__ import annotations
...
from src.experiments.protocol import load_config, run_protocol  # noqa: E402
...
def main() -> None:
    ...
    parser.add_argument("--config", default="configs/default.yaml")
    args = parser.parse_args()
    result = run_protocol(load_config(args.config))
    print(result)
...
def _compat_evaluate_script() -> str:
    return _compat_train_script()


def _agentic_smoke_test() -> str:
    return '''from __future__ import annotations

from src.method import MethodCore


def test_method_core_smoke() -> None:
    result = MethodCore().run({"toy": [1, 2, 3]})
    assert result["status"] == "ok"
    assert "paper_type" in result
'''


def _agentic_contract_test() -> str:
    return '''from __future__ import annotations

from pathlib import Path

from src.experiments.protocol import load_config, run_protocol
...
def _ml_adapter_code() -> str:
    return '''from __future__ import annotations


class OptionalMLAdapter:
    """Placeholder adapter for ML papers.

    Full PyTorch reproduction requires human-approved architecture, data, and
...
def _empty_module() -> str:
    return '"""No domain-specific adapter was generated for this paper type."""\n'


def _markdown_list(items: list[str]) -> str:
    return "\n".join(f"- {item}" for item in items) or "- Not extracted."


def _legacy_trace(trace: AgenticImplementationTrace) -> ImplementationTrace:
    return ImplementationTrace(
        entries=[
            ImplementationTraceEntry(
                step_id=entry.step_id,
                paper_claim=entry.claim,
                source=(
                    SourceSpan(
                        section=entry.evidence[0].section or entry.evidence[0].kind,
                        page=entry.evidence[0].page or 0,
                        text=entry.evidence[0].text,
                        chunk_id=entry.evidence[0].evidence_id,
                    )
                    if entry.evidence
                    else None
                ),
                implemented_in=entry.implemented_in,
